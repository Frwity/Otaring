Shader "Unlit/RadialProgress"
{
	Properties{
		_Frac("Progress Bar Value", Range(0,1)) = 1.0
		[NoScaleOffset] _AlphaTex("Alpha", 2D) = "White" {}
		_FillColor1("Fill Color1", Color) = (1,1,1,1)
		_FillColor2("Fill Color2", Color) = (1,1,1,1)
		_BackColor("Back Color", Color) = (0,0,0,1)
		_BlinkAlpha("Blink Alpha", Float) = 1.0
		...
		SubShader{
			...
			Pass {
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					// lerp between colors
					fixed4 col = lerp(lerp(_FillColor1, _FillColor2, _Frac), _BackColor, barProgress);

					fixed alpha = tex2D(_AlphaTex, i.uvMask.xy * 0.5 + 0.5).a;
					col.a *= alpha * _BlinkAlpha;
